...
class HTTPStatusChecker(client.HTTPClientFactory):
	def __init__(self,url,headers = None):
		...
		self.status = None
		self.deferred.addCallback(lambda data:(data,self.status,self.response_headers))
...
def checkStatus(url,contextFactory = None,*args,**kwargs):
	scheme,host,port,apth = client._parse(url)
	...
	if scheme == "https":
		...
		reactor.connnectSSL(host,port,factory,contextFactory)
	else:
...
def handleFirstResult(result,url):
	...
	return checkStatus(url,headers = nextRequestHeaders).addCallback(handleSecondResult)
...
def handleSecondResult(result):
	...
	if status == '200':
		print "Page change (or server dose not support conditional request)"
	elif status == '304':
...
if __name__ == "__main__":
	...
	checkStatus(url).addCallback(handleFirstResult,url).addErrback(handleError)	
	reactor.run()
